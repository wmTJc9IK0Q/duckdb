...
public:
	...
	static unique_ptr<MultiFileList> MultiFileFilterPushdown(ClientContext &context, const MultiFileBindData &data,
		...
		// A filter on a pushed-down struct extract cannot be evaluated against the parent column's
		// statistics. Omit it from file-list pruning; correctness outranks an I/O optimization.
